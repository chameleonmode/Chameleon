...
public partial class IdentityViewModel : ViewModelObjectBase {
	...
	private async Task OnSavePerson(UPPersonViewModel p) {
		...
		_ = await Task.Run(async() => _ = await UPAdditionalDataRepo.Save(UPAdditionalDataRepo.Instance.Personz, p.ToDto()));
	}
	...
	private async Task OnSaveBusiness(UPBusinessViewModel p) {
		_ = p.IsValidationValid();
		_ = await Task.Run(async () => _ = await UPAdditionalDataRepo.Save(UPAdditionalDataRepo.Instance.Biz, p.ToDto()));
		if (p.Id == 0)
	...
	private async Task OnSaveAddress(ObsAddressViewModel p) {
		_ = p.IsValidationValid();
		if (p.Dto != null) {
			_ = await Task.Run(async () => _ = await UPAdditionalDataRepo.Save(UPAdditionalDataRepo.Instance.Addrez, p.ToDto()));
		}
	}
