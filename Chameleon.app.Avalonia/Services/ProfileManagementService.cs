...
public interface IProfileManagementService {
	...
	Task EnsureProfilesLoadedAsync();
}

public class ProfileManagementService : IProfileManagementService {
	private bool areProfilesLoaded = false;
	...
	public async Task EnsureProfilesLoadedAsync() {
		var load = false;
		lock (_loadLock) {
			areProfilesLoaded = allProfiles.Any();
			load = !areProfilesLoaded;
		}
		if (load) {
			var dtos = await userProfilesRepo.GetAll<UserProfileDto>();
			ProfileDtoCache.Edit(update => {
				update.Clear();
