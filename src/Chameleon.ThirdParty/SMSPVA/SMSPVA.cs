...
public class SMSPVAService : IPVAInstance
{
    ...
    public string Name { get; } = "SMS PVA";
        ...
        where T2 : Models.Service
    {
        ...
        var jsonResponse = JsonSerializer.Deserialize<ApiResponse<GetNumberData>>(responseBody, JSOptions);
    ...
    public async Task<Tuple<string, string>> GetCodeAsync(RCountry country, RService app, string numberData)
    {
        ...
        var response =
           await ReceiveSMS(jsonResponse.Data.OrderId);

        var responseData =
           JsonSerializer.Deserialize<ApiResponse<ReceiveSMSData>>(response, JSOptions);

        return new Tuple<string, string>(response, responseData?.Data?.Sms?.Code);
    }
