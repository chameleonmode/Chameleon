...
using Chameleon.ThirdParty.SMSapi.SMSPVA.Models;
using System;
using System.Net.Http.Headers;
using System.Text.Json;

namespace Chameleon.ThirdParty.SMSapi.SMSPVA;

public class SMSPVAPI : PVAInstanceBase
{
    List<KeyValuePair<string, string>> ApiKeyHeaders => [ new("apikey", ApiKey) ];

    public override async Task Init()
    {
        var userSetting = await Task.Run(UserSettingsService.Instance.Get);
        ApiKey = userSetting.SmsPvaApiKey;
    }

    public override async Task Save()
    {
        var userSettingsService = UserSettingsService.Instance;
        var userSetting = await Task.Run(userSettingsService.Get);
        userSetting.SmsPvaApiKey = ApiKey;
        await Task.Run(() => userSettingsService.Save(userSetting));
    }
    ...
    async Task<Tuple<string, string>> GetActivationNumberAsync<T1, T2>(T1 country, T2 service)
        where T1 : Country
        where T2 : Service
    {
        var url =
        ...
        var responseBody =
            await HttpClientHelper.GetAsync(url, ApiKeyHeaders);
        var jsonResponse =
            JsonSerializer.Deserialize<ApiResponse<GetNumberData>>(responseBody, JSOptions);

        return new Tuple<string, string>(responseBody, jsonResponse?.Data?.PhoneNumber);
    }

    public override async Task<Tuple<string, string>> GetCodeAsync(RCountry country, RService app, string numberData)
    {
        var jsonResponse =
            JsonSerializer.Deserialize<ApiResponse<GetNumberData>>(numberData, JSOptions);
        var url =
            $"https://api.smspva.com/activation/sms/{jsonResponse.Data.OrderId}";
        var responseBody =
            await HttpClientHelper.GetAsync(url, ApiKeyHeaders);
        var responseData =
           JsonSerializer.Deserialize<ApiResponse<ReceiveSMSData>>(responseBody, JSOptions);

        return new Tuple<string, string>(responseBody, responseData?.Data?.Sms?.Code);
    }
    ...
    public override async Task<Tuple<string, string>> CancelOrderAsync(string orderId)
    {
        ...
        var responseContent = await HttpClientHelper.PutAsync(url, ApiKeyHeaders);
